...
from dashboard.models import Appointment,PopUpSubmission,ContactForm,Product,AvailableSlot
...
def appointment_form(request):
    if request.method == 'POST':
        ...
        from_email = 'info@kbgbd.com.au'  # replace with your email address
...
def popup(request):
    if request.method == 'POST':
        ...
        if request.COOKIES.get('pop_up_seen'):
            # Popup has been seen before, don't show it
            return render(request, 'index.html')
        else:
            # Popup has not been seen before, show it
